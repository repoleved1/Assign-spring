...
open class ZFRippleButton: UIButton {
    ...
    @IBInspectable open var rippleColor: UIColor = UIColor(red: 238.0/255.0, green: 238.0/255.0, blue: 238.0/255.0, alpha: 1) {
        didSet {
            rippleView.backgroundColor = rippleColor
        }
    }
    
    @IBInspectable open var rippleBackgroundColor: UIColor = UIColor.clear {
        didSet {
...
//            self.animateToNormal()
    ...
    func animateToRipple() {
        if trackTouchLocation {
            touchCenterLocation = self.center;
        } else {
            touchCenterLocation = nil
        }
...
//            self.animateToNormal()
        ...
        UIView.animate(withDuration: 0.03, delay: 0, options: [UIView.AnimationOptions.curveEaseOut, UIView.AnimationOptions.allowUserInteraction],
                       animations: {
                        self.rippleView.transform = CGAffineTransform.identity
        }, completion:{ (true) in
            self.animateToNormal();
