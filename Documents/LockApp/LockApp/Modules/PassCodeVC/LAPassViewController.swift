...
class LAPassViewController: UIViewController, TouchIDTableViewCellDelegate {
    ...
    class func instance() -> LAPassViewController {
        let vc = LAPassViewController()
        return vc
    }
...
extension LAPassViewController {
    ...
    func showErrorAlert(message: String) {
        Common.showAlert(message)
    }
    
    @available(iOS 10.0, *)
    func showGotoSettingsAlert(message: String) {
        _ = UIAlertController.present(style: .alert, title: message.localized, message: nil, attributedActionTitles: [("OK".localized, .default), ("Cancel".localized, .cancel)], handler: { (action) in
            if action.title == "OK".localized {
