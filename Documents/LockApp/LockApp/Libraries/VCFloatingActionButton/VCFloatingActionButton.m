...
-(id)initWithFrame:(CGRect)frame normalImage:(UIImage*)passiveImage andPressedImage:(UIImage*)activeImage withScrollview:(UIScrollView*)scrView
{
    ...
    if (self)
    {
        ...
        _menuTable = [[UITableView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH/4, -10, 0.75*SCREEN_WIDTH,SCREEN_HEIGHT - (SCREEN_HEIGHT - CGRectGetMaxY(self.frame)) )];
        _menuTable.scrollEnabled = NO;
        
        
        _menuTable.tableHeaderView = [[UIView alloc]initWithFrame:CGRectMake(0, -10, SCREEN_WIDTH/2, CGRectGetHeight(frame))];
    ...
    [UIView animateWithDuration:animationTime/2 animations:^
     {
         ...
         noOfRows = self->_labelArray.count;
...

{
    [UIView animateWithDuration:animationTime/2 animations:^
     ...
     } completion:^(BOOL finished)
     {
         ...
         [self->_mainWindow removeFromSuperview];
