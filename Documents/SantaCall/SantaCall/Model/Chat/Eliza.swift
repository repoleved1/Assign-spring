...
//            let matches = re.matches(in: statement, range: NSRange(location: 0, length: statement.characters.count))
            let matches = re.matches(in: statement, options: NSRegularExpression.MatchingOptions(rawValue: NSRegularExpression.MatchingOptions.anchored.rawValue), range: NSRange(location: 0, length: statement.count))
